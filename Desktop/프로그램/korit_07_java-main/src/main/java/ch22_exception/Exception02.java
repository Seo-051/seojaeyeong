...
public class Exception02 {
    ...
    public static void login(String id, String password) throws LoginFailedException {
        ...
        // 매개변수와 지역변수의 일치여부 확인하는 조건문
        // 메서드 내에 고전적인 예외 처리를 작성한 예씨
        if(!id.equals(correctId) || !password.equals(correctPassword)){
            //System.out.println("ID 혹은 비밀번호를 확인하세여");
            throw new LoginFailedException("ID 또는 PW가 틀렸습니다."); // "ID 또는 PW가 틀렸습니다."를
            // argument로 하는 LoginFailedExceptuon 객체가 생성되었다고 할 수 있음
        }
    }
    ...
    public static void main(String[] args) {
        ...
        //login(inputId, inputPassword);  // 코드 작성 순서 잘 기억하면 됨
        // throws 키워드를 달게 되니까 여기서 오류뜸

        try{
            login(inputId, inputPassword);
        } catch (LoginFailedException e){       // 자동완성으로 사용자 정의 Exception  클래스가 나왔는ㄴ데
            // 그 근거는 login() 메서드 다음에 throws LoginFailedException을 작성했기 때문임
            System.out.println("로그인 실패 : " + e.getMessage());
            // 이상을 실행했을때 e.getMessage()의 결과값이 "ID또는 PW가 틀렸습니다."라는 것을
            // 확인할 수 있습니다.
        } finally {
