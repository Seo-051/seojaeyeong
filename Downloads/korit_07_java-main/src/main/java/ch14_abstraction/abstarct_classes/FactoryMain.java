...
public class FactoryMain {
    public static void main(String[] args) {
        ...
        /*
            ...
            사실 우리는 교육 상황이기 때문에 모든 클래스를 한 번만 정의하고 그걸로 객체 한 두개 많들고 치우는
            경우가 많아서 모든 수업을 익명 클래스로 처리할 수도 있음

         */
        PhoneFatory phoneFatory1 = new PhoneFatory();
        phoneFatory1.setName("애플 스마트폰 공장");

        // 부모 클래스에서 선언한 추상 클래스를 자식 클래스에서 구현부 작성한대로 작동되는지 확인
        phoneFatory1.produce("아이폰17에어");
        phoneFatory1.manage();
        // 이번엔 부모 클래스의 일반 메서드를 호출
        phoneFatory1.showInfo();

        /*
            어제 오늘 배운 내용 생각했을 때 오버라이딩 개념은 '재정의'를 전제로
            부모 클래스와 자식 클래스의 메서드가 '다르게' 굴러갈 경우에만 이루어지면 되고,
            부모 클래스의 메서드를 동일하게 사용할거면 애초에 받아올 필요도 없이 그냥 쓰면 됩니다.

            다만 여러분이 자식 클래스를 정의 했을때 부모 클래스에 특정 메서드가 있음을 명확하게
            알고 있어야할 필요가 있다
        ...
        System.out.println("현재 공장은 " + tableFactory1.getName() + " 공장으로 변경되었습니다.");
        ...
        Factory factory2 = new Factory() {
            ...
            @Override
            public void manage() {
                System.out.println("메서드는 컴퓨터 공장을 관리합니다.");
            }
        };
