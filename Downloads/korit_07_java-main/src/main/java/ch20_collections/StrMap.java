...
public class StrMap {
    public static void main(String[] args) {
        ...
        // key 하나당 value는 하나입니다. 즉 키가 같다면 value 는 가장 최근 걸로 덮어쓰기 됩니다.
        strMap1.put("kor2025005", "KimFive");
        System.out.println(strMap1);

        /*
            List의 경우에는 순서가 있기 때문에 기본적으로 index를 통한 조회가 가능합니다.
            하지만 Map의 경우 아까 출력결과에서 본 것 처럼 순서를 보장하지 않기 때문에
        ...
        koreanScores1.put("김일", new Double[2]);         // 예시) 하지만 자료구조 복잡 이렇게도 사용가능


        Map<String, Double> koreanScores2 = new HashMap<>();
        koreanScores2.put("김일", 98.2);
        koreanScores2.put("김이", 100.2);
        koreanScores2.put("김삼", 9.2);
        koreanScores2.put("김사", 84.2);
        koreanScores2.put("김오", 46.2);

        // 특정 key의 value 수정
        // # 1. 아까 말한것처럼 key에는 value가 하나 밖에 없기 때문에 최근걸로 덮어쓰기 됩니다.
        System.out.println(koreanScores2);
        koreanScores2.put("김오", 4.4);
        System.out.println(koreanScores2);

        // # 2. .replace();
        koreanScores2.replace("김칠", 680.2);
        System.out.println(koreanScores2);
        /*
            두가지 방법이 있는 이유 .put()의 경우에는
            없는 key라면 key를 새로 생성해서 value를 초기화하겠지만
            있는 key리면 value에 재대입을 하게 됩니다.
        ...
        // 특정 value 존재 여부 -> containsValue(); -> boolan
        ...
        // 이상의 코드는 strMap1의 key-value 쌍을 set로 바꾸는 메서드인 .entrySet()의 호출 결과ㄹ,ㄹ
        ...
        // Map -> List로 직접 변환이 불가능하다는 예시

        entryList1.addAll(entrySet2);
        System.out.println(entryList1); //list로 바꿨으니까 element 추출이 가능함
        ...
        /*
            이상에서 강제 형변환 하지 않은 상태에서 오류가 발생하는 이유는 key는 어짜피 중복을 허용하지
            않기 때문에 Set으로 바꾸는 게 가능한 반면
            복수의 key에서 동일한 value가 존재할 수 있기 때문에 직접적으로 Set으로 바꾸지 않았음
         */
        Collection<String> values = strMap1.values();
        System.out.println(values);
        // 저희 Collection 자료형은 또 처음 써봄
        // .sort()는
        // Collections니까요
