...
/*
    ...
    이를 CharaterMain 클래스에서 기능 테스트를 수행합니다.
    ...
    스킬 세로베게
    exp 30
    으로 기본 생성자를 통해 생성하고, setter를 통해서 데이터를 집어넣으세요

    2. 메서드 정의
       getHealth() -> 체력 값을 반환하는 getter -> public / 이미 자동완성으로 생성
       attack() -> 공격시 콘솔 창에
            name 이 power로 공격 !
            name 이 경험치 amount를 얻었습니다. -> 라고 두줄 출력하게 만들것. call1() 유형
       heal() -> 체력을 10회복하고, 현재 체력 출력
            체력이 10회복 되었습니다. 현재 체력 : health(숫자로 출력)
       gainExp(int amount) -> call2() 유형이고 경험치를 증가하는 메서드(내부 전용)
             실행 예
             name이 경험치 amount를 얻음
...
class Character{
    ...
    int power;                  // 같은 패키지만 0개의 사용
    protected String skill;     //자식 클래스만
    private int exp;            //완전차단 0개의 사용위치

    // 생성자 (캐릭터 생성시 필수적으로 필요한 애들만 매개변수로 받은 매개변수 생성자)
    ...
    private void gainExp(int amount){
        exp += amount;
        System.out.println(name + "이 경험치를 " + amount + " 얻었습니다.");
    }
    public void heal(){
        health += 10;
...
//        warrior.gainExp(1000);  -> gainExp()를 private 처리 해서 오류
        //warrior.attack();
        //warrior.heal();
...
//        warrior.show();
        ...
        /*
            이상의 코드에서 여러분들이 중점적으로 학습해야 하는 부분은
            접근 지정자의 설정 수준에 따라서 setter/ getter가 필여한 사례가 있고 아닌 사례가 있다는 점
            예를 들어 게임을 하거나 sns를 한다고 가정했을때 닉네임의 경우는 누구나 볼 수 있다
            그렇다면 닉네임 필드는 public으로 설정되어있을겁니다.
            그렇기 때무네 getter를 통한 닉네임의 호출은 필요가 없어서 이상의 코드에서 getName() 메서드는 쓸모가 없음

            우리는 어떤 클래스/필드/메서드를 어떤 접근 지정자를 설절해야할지도 고민해야함
